...
class MaskTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.num_instances = 2 # self.model.num_instances
    ...
    def train_step(self, data):
        ...
        # pred_masks_flattened = pred_masks.view(-1) # [B*N, num_instances]
        ...
        # print("labeled.sum() = ", labeled.sum())
        if labeled.sum() > 0:
            pred_masks_labeled = pred_masks_flattened[labeled]
            gt_masks_labeled = gt_masks_flattened[labeled]

            # with np.printoptions(threshold=np.inf):
            #     print("pred_masks_labeled = ", pred_masks_labeled.cpu())

            # with np.printoptions(threshold=np.inf):
            #     print("gt_masks_labeled = ", gt_masks_labeled.cpu())
